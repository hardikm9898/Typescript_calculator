...
const trigon = document.querySelectorAll(".trigon");
...
button.forEach((btn) => btn.addEventListener("click", () => {
    ...
    switch (buttonValue) {
        case "=":
            ...
            for (let i = rightIndex.length - 1; i >= 0; i -= 1) {
                if (operator.includes(rightIndex.charAt(i))) {
                    ...
                    const answerValue = rightIndex.slice(indexLeft + 1, rightIndex.length + 1);
                    const value = rightIndex.slice(indexLeft + 1, rightIndex.length - 1);
                    displayText = displayText.replaceAll(answerValue, `${calculateFactorial(+value)}`);
                }
            }
                ...
                if (display.textContent) {
                    if (display.textContent.includes("=")) {
                        if (display.textContent.indexOf("=") !== -1) {
                            const NotPresentEqual = display.textContent.slice(1, display.textContent.length - 1);
        ...
        case "M+":
            if (display.textContent)
                if (display.textContent.includes("=")) {
                    memoryArray.push(this.eval(`+${display.textContent.slice(1, display.textContent.length - 1)}`));
                }
                else {
        ...
        case "M-":
            if (display.textContent)
                if (display.textContent.includes("=")) {
                    memoryArray.push(+`- ${display.textContent.slice(1, display.textContent.length - 1)}`);
                }
                else {
                    memoryArray.push(this.eval(display.textContent));
                }
